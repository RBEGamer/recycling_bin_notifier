...
{
    rbnurl = read_file(file_rbnurl, "192.168.1.70:3015");
}
...
void loop(void)
{
    ...
    if (millis() - last > 60 * 60 * 8)
    {
        if (WiFi.status() == WL_CONNECTED)
        {
            HTTPClient http;
            http.begin(rbnurl + "/rest/get_color_events_of_the_day"); //Specify request destination
        ...
        {
            last_error = "rest request WiFi.status() == WL_CONNECTED";
        }
    }
